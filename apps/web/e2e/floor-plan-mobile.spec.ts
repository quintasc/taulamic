...
test.describe('Plano — vista móvil', () => {
  ...
  test('controles arriba y lienzo compacto', async ({ page }) => {
    ...
    await expect(page.getByLabel('Ancho (m)').first()).toBeVisible();
    await expect(
      page.getByText('Medidas orientativas', { exact: false }),
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: 'Volver al tamaño recomendado' }),
    ).toBeVisible();
    await expect(
