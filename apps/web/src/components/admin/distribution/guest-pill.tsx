...
export function GuestPill({
  ...
  const pillClass = isRow
    ? `flex items-center justify-between w-full text-sm font-semibold text-neutral-800 ${
        canDrag
  ...
  const nameNode = isRow ? (
    <span className="flex-1 min-w-0 break-words whitespace-normal py-0.5 text-left pr-2">
      {name}
    </span>
  ) : (
  ...
  if (isRow) {
    return (
      <div className={`group/pill ${pillClass}`} {...dragProps}>
        {nameNode}
  ...
  return (
    <span className={`group/pill ${pillClass}`} {...dragProps}>
      {nameNode}
      {canRemove ? (
        <button
          type="button"
